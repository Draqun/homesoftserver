...
void *command_listener(void *args)
{
    while (listener_is_running)
    {
        ...
        {
        DELAY(9);
        char **device_inspection_statuses = (char**)NULL;
        {
        run_task(2, INXPECT_DEVICES, device_inspection_statuses);
        LOG_MESSAGE(DEBUG, "Devices inspected");
        }
        for (int i=0; i < CAMERAS && device_inspection_statuses && device_inspection_statuses[i]; ++i)
        {
            printf("%s", device_inspection_statuses[i]);
            free(device_inspection_statuses[i]);
        }
        free(device_inspection_statuses);
        }
    }
    return (void*)NULL;
}
